...
describe("SafeWallet", function() {
    ...
    describe("Transaction", async function () {
        it("Adding funds", async function () {
            expect(await this.owner.sendTransaction({
                to: wallet.address,
                value: 100,
        ...
        it("new transaction by owner", async function () {
            await expect(wallet.newTransaction(this.addr5.address, 50)).
                to.emit(wallet, "NewTransaction").withArgs(this.owner.address, 0, this.addr5.address ,50);
        })

        it("new transaction by other address SHOULD FAIL", async function () {
            await expect(wallet.connect(this.addr3).newTransaction(this.addr3.address, 50)).
        ...
        it("confirm with not owner SHOUD FAIL", async function () {
            await expect(wallet.connect(this.addr3).confirmTransaction(0)).to.be.revertedWith(/AccessControl: account .* is missing role .*/);
        })

        it("confirm with owner", async function () {
            await expect(wallet.connect(this.addr1).confirmTransaction(0)).to.emit(wallet, "ConfirmTransaction").withArgs(this.addr1.address, 0);
            await expect(wallet.confirmTransaction(0)).
                to.changeEtherBalance(this.addr5, 50).
                to.emit(wallet,"ConfirmTransaction").withArgs(this.owner.address, 0).
                to.emit(wallet, "ExecuteTransaction").withArgs(0);
        })
    })
})